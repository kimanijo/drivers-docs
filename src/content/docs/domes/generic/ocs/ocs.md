...
The OCS is an open-source (hardware and software) Observatory control that is very flexible and customizable.  
[https://onstep.groups.io/g/onstep-ocs/wiki](https://onstep.groups.io/g/onstep-ocs/wiki)  for full info.
...
**Configuration**  
Connection via USB and Ethernet is supported.  
...
**  
Main Control**  
![Main Control Tab](./images/Main_Control_Tab.webp"Main Control Tab")
...
- Parking - Park(ed)/UnPark(ed) control/indication. Sends the dome to the defined park position/removes the dome from parked status.  
- Additional Controls  
...
Connection  
![Connection Tab](./images/Connection_Tab.webp "Connection Tab")
...
**Options**  
![Options Tab](./images/Options_Tab.webp "Options Tab")
...
- Shutter > Close on Park, Open on UnPark. Note that this option is independent of the OCS option ROOF\_CLOSE\_PARKS\_MOUNT. If that option is set in the OCS configuration it is always active. These controls act in addition to that option and are not aware of it.

**Presets**  
![Presets Tab](./images/Presets_Tab.webp "Presets Tab")

Allows creation and GoTo upto three stored locations of dome Azimuth.

**Slaving**  
![Slaving Tab](./images/Slaving_Tab.webp"Slaving Tab")
...
**Status**  
![Status Tab](./images/Status_Tab.webp "Status Tab")

Shows miscellaneous details from the OCS.

- Firmware version - As reported by OCS. The driver expects a minimum version and will report if the connect OCS is below this.  
...
**Weather**  
![Weather Tab](./images/Weather_Tab.webp "Weather Tab")
...
**Note that the OCS internal safety status is independent of these driver settings. OCS considers Rain, Cloud cover, Wind speed & Mains power present towards it's safety status (if they are configured) and will operate independent of these settings and driver connection. Safe limits defined in the OCS software for Wind speed and Cloud cover are copied into the driver as initial limits upon connection but may be over-ridden here. In addition, temperature, pressure & humidity are considered towards the Indi safety status. If not required for this but still to be monitored, just set the min/max limits very wide so that they are always 'safe'.**

**Thermostat**  
![Thermostat Tab](./images/Thermostat_Tab.webp"Thermostat Tab")
...
**Power**  
![Power Tab](./images/Power_Tab.webp "Power Tab")
...
**Lights**  
![Lights Tab](./images/Lights_Tab.webp "Lights Tab")

Any OCS-configured lighting device is shown with ON/OFF controls/status indications that activate the defined relay.

**Manual**  
![Manual Tab](./images/Manual_Tab.webp"Manual Tab")

**Note the warning at the top of this tab - these are controls that directly address the hardware in ways that could result in lost data, damage to equipment, or in the worst case unusual movement of equipment that has the potential for injury.**
